...
public:
    long long maximumTotalDamage(vector<int> &power)
    {
        ...
        vector<int> uniqueDamages = getSortedUniqueDamages(count);
        const int n = uniqueDamages.size();
        // dp[i][k] := the maximum damage using uniqueDamages[0..i],
        // where k indicates if the i-th damage is used
        vector<vector<long long>> dp(n, vector<long long>(2));

        for (int i = 0; i < n; ++i)
        {
            const long long damage = uniqueDamages[i];
            if (i == 0)
            {
                dp[0][0] = 0;
