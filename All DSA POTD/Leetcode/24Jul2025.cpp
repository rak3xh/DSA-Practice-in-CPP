...
public:
    int minimumScore(vector<int> &nums, vector<vector<int>> &edges)
    {
        ...
        // Replace std::reduce with manual XOR accumulation
        int xors = 0;
        for (int num : nums)
            xors ^= num;
        ...
        for (int i = 0; i < edges.size(); ++i)
        {
            ...
            for (int j = 0; j < i; ++j)
            {
                ...
                int maxVal = *max_element(cands.begin(), cands.end());
                int minVal = *min_element(cands.begin(), cands.end());
...
private:
                                      ...
                                      vector<unordered_set<int>> &children)
    {
        for (int v : tree[u])
        {
            ...
            pair<int, unordered_set<int>> result = dfs(tree, v, u, subXors, children);
            int vXor = result.first;
            unordered_set<int> &vChildren = result.second;
