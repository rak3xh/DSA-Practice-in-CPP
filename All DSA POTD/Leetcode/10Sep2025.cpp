...
public:
    int minimumTeachings(int n, vector<vector<int>> &languages, vector<vector<int>> &friendships)
    {
        ...
        for (const vector<int> &language : languages)
            languageSets.push_back(unordered_set<int>(language.begin(), language.end()));
        ...
        // Count the most popular language among those who need to be taught.
        ...
        return (int)needTeach.size() - maxCount;
    }
...
private:
    ...
    bool cantTalk(const vector<unordered_set<int>> &languageSets, int u, int v)
    {
        for (const int language : languageSets[u])
        {
            if (languageSets[v].find(language) != languageSets[v].end())
                return false;
        }
        return true;
    }
