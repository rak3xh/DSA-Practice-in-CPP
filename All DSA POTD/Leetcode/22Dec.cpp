...
public:
    vector<int> leftmostBuildingQueries(vector<int> &heights, vector<vector<int>> &queries)
    {
        ...
        vector<int> stack; // Monotonic stack
        ...
        for (const auto &query : getIndexedQueries(queries))
        {
            ...
            else
            {
                ...
                auto it = upper_bound(stack.rbegin(), stack.rend(), a,
                                      [&](int aIndex, int stackIndex)
                                      {
                                          return heights[aIndex] < heights[stackIndex];
                                      });
...
private:
    vector<IndexedQuery> getIndexedQueries(const vector<vector<int>> &queries)
    {
        ...
        sort(indexedQueries.begin(), indexedQueries.end(),
             [](const IndexedQuery &q1, const IndexedQuery &q2)
             {
                 return q1.b > q2.b;
             });
