...
public:
    int maximumScore(vector<int> &nums, int k)
    {
        ...
        sort(numAndIndexes.begin(), numAndIndexes.end(), [](const pair<int, int> &a, const pair<int, int> &b)
             { return a.first == b.first ? a.second < b.second : a.first > b.first; });

        for (const auto &p : numAndIndexes)
        {
            int num = p.first, i = p.second;
            long rangeCount = static_cast<long>(i - left[i]) * (right[i] - i);
            long actualCount = min(rangeCount, static_cast<long>(k));
            k -= actualCount;
...
private:
    ...
    vector<int> getPrimeScores(const vector<int> &nums, const vector<int> &minPrimeFactors)
    {
