...
public:
    vector<int> earliestAndLatest(int n, int firstPlayer, int secondPlayer)
    {
        ...
        vector<vector<vector<P>>> mem(n + 1, vector<vector<P>>(n + 1, vector<P>(n + 1)));

        pair<int, int> res = solve(firstPlayer, n - secondPlayer + 1, n, mem);
        int a = res.first;
        int b = res.second;
...
private:
    // Returns the (earliest, latest) pair.
        ...
        for (int i = 1; i <= l; ++i)
        {
            for (int j = l - i + 1; j <= r - i; ++j)
            {
                if (i + j > (k + 1) / 2 || i + j < l + r - k / 2)
                    continue;
                pair<int, int> res = solve(i, j, (k + 1) / 2, mem);
                a = min(a, res.first + 1);
                b = max(b, res.second + 1);
            }
        }
